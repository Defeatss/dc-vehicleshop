...
$(document).on('click', ".color", function() {
	...
	$.post("https://dc-vehicleshop/changeColor",JSON.stringify({colorR,colorG,colorB}));
});
$(document).on('click', ".category, .category5", function() {
	...
	$("#colors-button").text("Colour options");
	...
	$.post("https://dc-vehicleshop/spawnVehicle",JSON.stringify({model}));
});
...
$("#test-accept").click(function() {
	let timer = data.testDrive.testDriveTimer;
	$.post("https://dc-vehicleshop/testDrive",JSON.stringify({timer,details}))
})
...
$("#buy").click(function() {
	...
	$.post("https://dc-vehicleshop/buyVehicle",JSON.stringify({details}));
})
	...
	if (key == "Escape"){
		if(!onBuyPage){
			if(vehiclesCategory){
				...
				$.post("https://dc-vehicleshop/deletevehicle",JSON.stringify({}));
			}else{
				$("#vehicle-class-title-container").fadeOut();
				$("#vehicle-class-container").fadeOut();
				$.post("https://dc-vehicleshop/closeVehicleShop",JSON.stringify({}));
			}
		}
...
$("#hide-button").click(function(){
	if(!hideDisplay){
		...
		$.post("https://dc-vehicleshop/zoomCam",JSON.stringify({}));
	}else{
		console.log("dsa")
		hideDisplay = false;
		inspect = false;
		$("#hide-button").css("margin-top","2px");
		$.post("https://dc-vehicleshop/returnCam",JSON.stringify({}));
		setTimeout(() => {
			$("#vehicle-container").fadeIn();
			$("#title-container").fadeIn();
...
//     $.post("https://dc-vehicleshop/changePos",JSON.stringify({data}));
// }
...
document.onmousedown = function(e){
	if(inspect){
		if (vehicleDisplay && !down && e.button == 0){
			down = true;
			$.post("https://dc-vehicleshop/mousedown",JSON.stringify({}));
		}
	}
}
...
document.onmouseup = function(e){
	if(inspect){
		if (down && e.button == 0){
			$.post("https://dc-vehicleshop/mouseup",JSON.stringify({}))
			down = false;
		}
	}
}
...
window.onmousewheel = function(e){
	if(inspect){
		...
		$.post("https://dc-vehicleshop/" + (st < 0 ? "downscroll" : "upscroll"),JSON.stringify({}));
	}
}
