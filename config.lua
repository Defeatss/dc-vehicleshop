...
Config.Colors = {
  ...
  {r = 102, g = 184, b = 31, colorName = "Light Green", gtaColor = 55},
  {r = 110, g = 163, b = 198, colorName = "Light Blue", gtaColor = 74},
}
...
Config.BuyVehicleFunc = function(Core,vehicleEntity,vehicleName)
  --[[
    ...
    TriggerServerEvent('dc-vehicleshop:setVehicleOwned', vehicleProps)
end
...
Config.ResetStockEvent = {
    name = "dc-vehicleshop:resetstock", -- This event resets the vehicles' stock. [*Server Side*],
    safe_for_net = false --[[
...
Config.SetDefaultStockEvent = {
  name = "dc-vehicleshop:setDefaultStock", -- This event generates the stock.json file with values from the config.lua [*It is recommended to do it on first launch.]
  safe_for_net = true --[[
    true = allows you to trigger the event from a client side script and from a server side script.
    false =  allows you to trigger the event from a server side script only.
  ]]
